...
public enum Kit {
	...
	DEMON("HELLFIRE", 0, 15, Material.MAGMA, Arrays.asList("&7The Demon kit uses fire","&7to destroy their targets.","&aLeap: Jump while sneaking to launch yourself."), (new InventoryBuilder(null, InventoryType.PLAYER).addItem(new ItemCreator(Material.GOLD_SWORD).addEnchant(Enchantment.FIRE_ASPECT, 2).getItem()).getInventory()).getStorageContents(), new ItemStack(Material.GOLD_HELMET, 1), new ItemCreator(Material.LEATHER_CHESTPLATE).setDyeColor(DyeColor.RED).getItem(), null, new ItemCreator(Material.LEATHER_BOOTS).setDyeColor(DyeColor.RED).getItem(), null, new PotionEffect(PotionEffectType.REGENERATION, 100, 1)),
